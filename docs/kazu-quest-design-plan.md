...
- **2026-09-24 (全体のブラッシュアップ)**:
  - **章2 (小2) の文にも ルビ**: 塔・魔女・弐・港・砂・僧侶 (23行)。読みやすさの回帰テストを
    章1 (小1漢字) → 章1・2 (小1+小2漢字) に広げた
  - **小4「グラフ」単元に ぼうグラフ**: 新しい図 `barChart` (`figures/BarChart.tsx`)。以前は
    文の「ひょう」だけで グラフが一度も出なかった。文のひょうは まちがいノート用に残す
  - **くりかえしの多い問題**: 小3 分数 Lv1 は 2問 (1/2・1/3) しかなかった → 2〜6等分の
    いくつ分 (15種以上)。小4 大きな数 は こたえが 3割「10000」→「n兆は 1億の なんこ分?」に
  - **FieldScene を 800行未満に**: UI だけで完結する effect の振り分けを
    `field/effectDispatch.ts` へ (シーンを動かす transfer/savePoint/battle/ending は残す)
