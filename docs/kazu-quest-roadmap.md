...
| フェーズ | ねらい | タスク | 目安 |
|---|---|---|---|
| P0 安全網 | 55マップの回帰検出と数値バランスの根拠づくり | KQ-01〜07 | 7セッション |
...
#### KQ-02 [M] 章3 ゴールデンパス E2E — 状態: 未 (依存: KQ-01)
...
#### KQ-07 [M] バランス・シミュレータ (純ロジック) — 状態: 未
...
#### KQ-11 [M] とっくん (ヒントつき練習) + さくらんぼ図 — 状態: 未 (依存: KQ-10)
...
#### KQ-12 [M] テンキー入力モード (小3以降のテスト・おだい・とっくん) — 状態: 未 (依存: KQ-11)
...
#### KQ-13 [S] ふくしゅうのほこら (弱点3単元の復習クエスト) — 状態: 未
...
#### KQ-20 [M] 効果音 (アセット0 WebAudio) — 状態: 未
...
#### KQ-21 [M] BGM (チップチューン・シーケンサ) — 状態: 未 (依存: KQ-20)
...
#### KQ-22 [M] エンディング演出 + タイトルメニュー + クリア後状態 — 状態: 未
...
#### KQ-23 [S] ボス部屋前の すいしょうレベル案内 — 状態: 未 (依存: KQ-07)
...
#### KQ-25 [M] ルビのエンジン対応 (章3以降の本文用) — 状態: 未
...
#### KQ-30a [M] 章の出題プールを学年から切り離す (`questionGrades`) — 状態: 未
...
#### KQ-30b [L] 終章「ムゲンのらせん」+ 隠しボス ∞竜ムゲニア — 状態: 未 (依存: KQ-30a, KQ-22)
...
#### KQ-31 [S] NPC ミニクイズ + ひらめきメダル交換所 — 状態: 未 (依存: KQ-13 のメダル)
...
#### KQ-41 [S] E2E の CI 実行 (サブセット) — 状態: 未 (依存: KQ-01)
...
#### KQ-42 [S] 本番プレイスルー・チェックリスト + docs 同期 — 状態: 未
