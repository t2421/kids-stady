...
2. `apps/<slug>/index.html` の `<title>` と本文を、そのアプリ用に書き換える。
   - 見た目はそのアプリの世界観に合わせて自由に作る (下記「設計方針」参照)。
     `shared/css/tokens.css` の `.pill` `.overlay` `.bigbtn` などは、世界観に合うなら
     使ってよい程度の道具と考える。
...
- **共通化するのはデータ関連だけ**: アプリ間で共有するのは
  プレイヤープロフィール (`kidsStudy.profiles.v1`) と学習履歴・解答テレメトリの契約
  (`recordAnswer(skillId, correct, ms)` / `skillStats` / `history` — `docs/save-data.md`)。
  ゲームロジックは共通化しない (アプリごとに独立させ、1本の変更が他に影響しないようにする)。
- **プレイヤー選択は全アプリ必須・同じ流れにする**: 起動時に「だれが あそぶ?」で
  プロフィールを選択 (一覧から選ぶ / あたらしく つくる=なまえ+アバター / けす=2段階確認)、
  ゲーム中に「ちがう ひとが あそぶ」で切替できること。実装はアプリごと
  (見た目はそのアプリの世界観でよい) だが、流れ・文言はこの形に揃え、
  データは共有プロフィール契約を使う。参考実装: `apps/keisan-shooter` (vanilla) /
  `apps/kazu-quest/src/components/ProfileGate.tsx` (React)。
- **見た目・テイストは合わせない**: 各ゲームは**それぞれの世界観に合わせたビジュアル**を持つ
  (例: けいさんシューターはポップな3D、カズクエはファミコンDQ風ドット絵)。
  `shared/css/tokens.css` や `KidsAudio` はハブページと簡易アプリ向けの道具であって、
  全アプリで統一デザインにするためのものではない。使うかどうかはアプリの世界観で決める。
