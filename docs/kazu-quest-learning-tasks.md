...
#### LP-23 [M] E2E 総仕上げ — 状態: 済

- `e2e/learning.spec.ts`: 小1・小3・小5 の中核単元をレッスンから合格まで通す (既存の
  lesson-grade1/3/5.spec.ts で充足済み、重複させず) / 前提不足の案内 / おさらい → マスター /
  さきどり (小1 のセーブで小2 の単元を学ぶ) / 章ゲート (can 未満で番人が残る)。
  `e2e.yml` のスモークに `lesson.spec.ts` を追加。受け入れ: フル E2E 緑 (55件、default+ipad両方)
  — 実装済み・全通し確認済み。
  **発覚した既知の逸脱**: handleOpenLesson (lessonFlow.ts) の readiness ゲート分岐
  (前提不足 → open-readiness → 不合格なら weakest のレッスンへ回り道) は、現在の実プレイ
  導線からは到達不能なデッドコード — まなびやの先生メニュー (TeacherMenu.tsx) は前提を
  経由せず直接 open-lesson、さきどり (PreviewMenu.tsx) は前提充足済みのものしか一覧に
  出さない、なかまが教える場面は skipReadiness:true 固定。ロジック自体は
  tests/lessonFlow.test.ts で健在だが、実際にプレイヤーが前提不足の案内を目にする経路が
  今は無い。修正には新しい導線 (例えば まなびやメニューで前提未充足の単元もあえて
  一覧に出し、タップ時に readiness を挟む) が要るが、LP-18/19 の設計判断 (前提を気にせず
  即開く) と衝突するため、別タスクとして再検討が必要。
