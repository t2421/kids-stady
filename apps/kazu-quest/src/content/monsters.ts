/*
 * モンスター定義 (章横断)。数値の初期バランスは
 * docs/kazu-quest-design-plan.md A7 に準拠。
 */

import type { MonsterDef } from "./types";

export const MONSTERS: Record<string, MonsterDef> = {
  keshigomun: {
    id: "keshigomun",
    name: "ケシゴムン",
    art: "keshigomun",
    hp: 8,
    atk: 5,
    def: 1,
    agi: 2,
    exp: 2,
    gold: 1,
    actions: [{ kind: "attack", weight: 1 }],
  },
  inkugumo: {
    id: "inkugumo",
    name: "インクぐも",
    art: "inkugumo",
    hp: 12,
    atk: 6,
    def: 2,
    agi: 3,
    exp: 3,
    gold: 2,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  kazunezumi: {
    id: "kazunezumi",
    name: "かずぬすみネズミ",
    art: "kazunezumi",
    hp: 10,
    atk: 6,
    def: 1,
    agi: 6,
    exp: 3,
    gold: 3,
    actions: [{ kind: "attack", weight: 1 }],
  },
  mojibakeBat: {
    id: "mojibakeBat",
    name: "モジバケバット",
    art: "mojibakeBat",
    hp: 14,
    atk: 7,
    def: 2,
    agi: 8,
    exp: 4,
    gold: 3,
    actions: [{ kind: "attack", weight: 1 }],
  },
  togeImomushi: {
    id: "togeImomushi",
    name: "とげとげイモムシ",
    art: "togeImomushi",
    hp: 18,
    atk: 8,
    def: 3,
    agi: 2,
    exp: 5,
    gold: 4,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  dekaInkugumo: {
    id: "dekaInkugumo",
    name: "でかインクぐも",
    art: "dekaInkugumo",
    hp: 70,
    atk: 11,
    def: 3,
    agi: 4,
    exp: 20,
    gold: 30,
    actions: [
      { kind: "attack", weight: 2 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  /* ---------- 第2章 (海辺と九九の塔) ---------- */
  awaKeshigomun: {
    id: "awaKeshigomun",
    name: "あわケシゴムン",
    art: "awaKeshigomun",
    hp: 16,
    atk: 9,
    def: 3,
    agi: 5,
    exp: 7,
    gold: 5,
    actions: [{ kind: "attack", weight: 1 }],
  },
  inkgani: {
    id: "inkgani",
    name: "インクガニ",
    art: "inkgani",
    hp: 20,
    atk: 10,
    def: 6,
    agi: 3,
    exp: 9,
    gold: 7,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  shuseiekin: {
    id: "shuseiekin",
    name: "シュウセイエキン",
    art: "shuseiekin",
    hp: 24,
    atk: 11,
    def: 4,
    agi: 7,
    exp: 11,
    gold: 9,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  blotta: {
    id: "blotta",
    name: "インクの魔女ブロッタ",
    art: "blotta",
    hp: 240,
    atk: 21,
    def: 6,
    agi: 8,
    exp: 180,
    gold: 350,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
      { kind: "heal", weight: 1 },
    ],
  },
  /* ---------- 第3章 (砂の国ワケーラ) ---------- */
  sunaKeshigomun: {
    id: "sunaKeshigomun",
    name: "すなケシゴムン",
    art: "sunaKeshigomun",
    hp: 28,
    atk: 14,
    def: 5,
    agi: 6,
    exp: 16,
    gold: 12,
    actions: [{ kind: "attack", weight: 1 }],
  },
  sunanezumi: {
    id: "sunanezumi",
    name: "すなぬすみネズミ",
    art: "sunanezumi",
    hp: 26,
    atk: 13,
    def: 4,
    agi: 12,
    exp: 15,
    gold: 18,
    actions: [{ kind: "attack", weight: 1 }],
  },
  cactusKnife: {
    id: "cactusKnife",
    name: "サボテンナイフ",
    art: "cactusKnife",
    hp: 34,
    atk: 16,
    def: 8,
    agi: 5,
    exp: 20,
    gold: 15,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  sasoriCompass: {
    id: "sasoriCompass",
    name: "さそりコンパス",
    art: "sasoriCompass",
    hp: 30,
    atk: 18,
    def: 6,
    agi: 10,
    exp: 22,
    gold: 16,
    actions: [
      { kind: "attack", weight: 2 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  mummyFusen: {
    id: "mummyFusen",
    name: "ミイラふせん",
    art: "mummyFusen",
    hp: 40,
    atk: 17,
    def: 9,
    agi: 4,
    exp: 26,
    gold: 20,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  wakemaeGolem: {
    id: "wakemaeGolem",
    name: "わけまえゴーレム",
    art: "wakemaeGolem",
    hp: 280,
    atk: 75,
    def: 12,
    agi: 3,
    exp: 90,
    gold: 90,
    actions: [
      { kind: "attack", weight: 2 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  amarida: {
    id: "amarida",
    name: "盗賊王アマリダ",
    art: "amarida",
    hp: 450,
    atk: 50,
    def: 10,
    agi: 14,
    exp: 400,
    gold: 700,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
      { kind: "heal", weight: 1 },
    ],
  },
  /* ---------- 第4章 (氷の国メジャーリア) ---------- */
  yukiKeshigomun: {
    id: "yukiKeshigomun",
    name: "ゆきケシゴムン",
    art: "yukiKeshigomun",
    hp: 52,
    atk: 24,
    def: 12,
    agi: 8,
    exp: 44,
    gold: 30,
    actions: [{ kind: "attack", weight: 1 }],
  },
  kooriBat: {
    id: "kooriBat",
    name: "こおりバット",
    art: "kooriBat",
    hp: 48,
    atk: 26,
    def: 10,
    agi: 18,
    exp: 48,
    gold: 32,
    actions: [{ kind: "attack", weight: 1 }],
  },
  yukiDaruman: {
    id: "yukiDaruman",
    name: "ゆきだるマン",
    art: "yukiDaruman",
    hp: 70,
    atk: 27,
    def: 16,
    agi: 5,
    exp: 60,
    gold: 40,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  bundokiKani: {
    id: "bundokiKani",
    name: "ぶんどきガニ",
    art: "bundokiKani",
    hp: 62,
    atk: 28,
    def: 18,
    agi: 9,
    exp: 58,
    gold: 38,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  monosashiOokami: {
    id: "monosashiOokami",
    name: "ものさしオオカミ",
    art: "monosashiOokami",
    hp: 66,
    atk: 31,
    def: 13,
    agi: 20,
    exp: 66,
    gold: 44,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  kooriGolem: {
    id: "kooriGolem",
    name: "こおりのゴーレム",
    art: "kooriGolem",
    hp: 480,
    atk: 120,
    def: 22,
    agi: 6,
    exp: 220,
    gold: 200,
    actions: [
      { kind: "attack", weight: 2 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  decimaron: {
    id: "decimaron",
    name: "小数の魔人デシマロン",
    art: "decimaron",
    hp: 750,
    atk: 75,
    def: 18,
    agi: 22,
    exp: 900,
    gold: 1400,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  /* ---------- 第5章 (割合の都と 魔王マイナドス) ---------- */
  hasuuKeshigomun: {
    id: "hasuuKeshigomun",
    name: "はすうケシゴムン",
    art: "hasuuKeshigomun",
    hp: 92,
    atk: 42,
    def: 22,
    agi: 14,
    exp: 150,
    gold: 90,
    actions: [{ kind: "attack", weight: 1 }],
  },
  waribikiGhost: {
    id: "waribikiGhost",
    name: "ワリビキゴースト",
    art: "waribikiGhost",
    hp: 100,
    atk: 45,
    def: 18,
    agi: 24,
    exp: 175,
    gold: 120,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  tsuubunSnake: {
    id: "tsuubunSnake",
    name: "ツウブンヘビ",
    art: "tsuubunSnake",
    hp: 115,
    atk: 48,
    def: 24,
    agi: 20,
    exp: 195,
    gold: 130,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  taisekiCube: {
    id: "taisekiCube",
    name: "タイセキキューブ",
    art: "taisekiCube",
    hp: 140,
    atk: 46,
    def: 34,
    agi: 8,
    exp: 210,
    gold: 140,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  kaiteiKani: {
    id: "kaiteiKani",
    name: "カイテイガニ",
    art: "kaiteiKani",
    hp: 120,
    atk: 50,
    def: 30,
    agi: 16,
    exp: 200,
    gold: 135,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  kumoNoBanjin: {
    id: "kumoNoBanjin",
    name: "くもの ばんじん",
    art: "kumoNoBanjin",
    hp: 950,
    atk: 125,
    def: 30,
    agi: 18,
    exp: 480,
    gold: 400,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  shinkaiNoNushi: {
    id: "shinkaiNoNushi",
    name: "しんかいの ぬし",
    art: "shinkaiNoNushi",
    hp: 800,
    atk: 100,
    def: 26,
    agi: 22,
    exp: 540,
    gold: 450,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
      { kind: "heal", weight: 1 },
    ],
  },
  minados: {
    id: "minados",
    name: "魔王マイナドス",
    art: "minados",
    hp: 1000,
    atk: 85,
    def: 32,
    agi: 30,
    exp: 2000,
    gold: 3000,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  /* ---------- 第6章 (下の世界ネガリア) ---------- */
  zeroKeshigomun: {
    id: "zeroKeshigomun",
    name: "ゼロケシゴムン",
    art: "zeroKeshigomun",
    hp: 180,
    atk: 70,
    def: 40,
    agi: 24,
    exp: 500,
    gold: 260,
    actions: [{ kind: "attack", weight: 1 }],
  },
  negaGhost: {
    id: "negaGhost",
    name: "ネガゴースト",
    art: "negaGhost",
    hp: 200,
    atk: 76,
    def: 36,
    agi: 34,
    exp: 560,
    gold: 300,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  hayasaBat: {
    id: "hayasaBat",
    name: "ハヤサバット",
    art: "hayasaBat",
    hp: 170,
    atk: 72,
    def: 30,
    agi: 46,
    exp: 540,
    gold: 280,
    actions: [{ kind: "attack", weight: 1 }],
  },
  sokudoWolf: {
    id: "sokudoWolf",
    name: "ソクドオオカミ",
    art: "sokudoWolf",
    hp: 210,
    atk: 82,
    def: 38,
    agi: 42,
    exp: 640,
    gold: 340,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  zeroCube: {
    id: "zeroCube",
    name: "ゼロキューブ",
    art: "zeroCube",
    hp: 260,
    atk: 78,
    def: 52,
    agi: 14,
    exp: 700,
    gold: 380,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  maboroshiHero: {
    id: "maboroshiHero",
    name: "まぼろしの ゆうしゃ",
    art: "maboroshiHero",
    hp: 1050,
    atk: 120,
    def: 46,
    agi: 40,
    exp: 1500,
    gold: 900,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
      { kind: "heal", weight: 1 },
    ],
  },
  zerom: {
    id: "zerom",
    name: "冥王ゼロム",
    art: "zerom",
    hp: 900,
    atk: 92,
    def: 50,
    agi: 38,
    exp: 3000,
    gold: 4000,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  zeromTrue: {
    id: "zeromTrue",
    name: "冥王ゼロム・しんのすがた",
    art: "zeromTrue",
    hp: 1400,
    atk: 105,
    def: 56,
    agi: 48,
    exp: 6000,
    gold: 8000,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 4 },
      { kind: "heal", weight: 1 },
    ],
  },
  /* ---------- 終章「ムゲンのらせん」(KQ-30b) — 章6より ひとまわり つよい 色ちがい ---------- */
  rasenKeshigomun: {
    id: "rasenKeshigomun",
    name: "ラセンケシゴムン",
    art: "rasenKeshigomun",
    hp: 230,
    atk: 84,
    def: 46,
    agi: 28,
    exp: 720,
    gold: 360,
    actions: [{ kind: "attack", weight: 1 }],
  },
  rasenBat: {
    id: "rasenBat",
    name: "ラセンバット",
    art: "rasenBat",
    hp: 210,
    atk: 86,
    def: 36,
    agi: 52,
    exp: 760,
    gold: 380,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  rasenDaruman: {
    id: "rasenDaruman",
    name: "ラセンダルマン",
    art: "rasenDaruman",
    hp: 300,
    atk: 90,
    def: 58,
    agi: 12,
    exp: 860,
    gold: 420,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "heal", weight: 1 },
    ],
  },
  rasenSnake: {
    id: "rasenSnake",
    name: "ラセンヘビ",
    art: "rasenSnake",
    hp: 260,
    atk: 94,
    def: 44,
    agi: 40,
    exp: 820,
    gold: 400,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
    ],
  },
  rasenCube: {
    id: "rasenCube",
    name: "ラセンキューブ",
    art: "rasenCube",
    hp: 320,
    atk: 92,
    def: 62,
    agi: 16,
    exp: 900,
    gold: 440,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 1 },
    ],
  },
  /*
   * ∞竜ムゲニア — 隠しボス (1形態)。HP は ゼロム真の姿の 1.3 倍、atk/def/agi は同格 (KQ-30b)。
   * シミュ (章6装備・4人): Lv42 60% / Lv45 78% / Lv48 82% (tests/balance.test.ts)
   */
  mugenia: {
    id: "mugenia",
    name: "∞竜ムゲニア",
    art: "mugenia",
    hp: 1820,
    atk: 100,
    def: 54,
    agi: 48,
    exp: 12000,
    gold: 15000,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 4 },
      { kind: "heal", weight: 1 },
    ],
  },
  eraser: {
    id: "eraser",
    /* 章1 (小1) のボス。戦闘の文字は canvas で ルビが付けられないので ひらがなで書く */
    name: "かんぶイレイサー",
    art: "eraser",
    hp: 90,
    atk: 12,
    def: 4,
    agi: 5,
    exp: 60,
    gold: 100,
    actions: [
      { kind: "attack", weight: 3 },
      { kind: "strongAttack", weight: 2 },
      { kind: "heal", weight: 1 },
    ],
  },
};

export function getMonster(id: string): MonsterDef | undefined {
  return MONSTERS[id];
}
